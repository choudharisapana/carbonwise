...
.addRepository(
  ...
  const calculateStats = () => {
    ...
    const analyzed = repositories.filter(r => r.lastAnalyzed).length;
    const avgSustainability = Math.round(
      repositories.reduce((sum, r) => sum + (r.sustainabilityScore || 0), 0) / total
    );
    const avgCarbon = Math.round(
      repositories.reduce((sum, r) => sum + (r.carbonEmission || 0), 0) / total
    );
